...
For pnpm, Yarn, or Bun installs, use the matching global remove command: `pnpm remove -g asuka.pi`, `yarn global remove asuka.pi`, or `bun uninstall -g asuka.pi`.
...
Configure a provider in `~/.pi/agent/config.json`, then set one of the credential variables declared by that provider or store its API key with `/login`.
...
- [Providers](providers.md) - configured providers, authentication, protocols, and exact endpoints.
...
- [Custom providers](custom-provider.md) - register extension-owned streaming or authentication behavior.
